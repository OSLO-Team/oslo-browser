...
document.getElementById('btn-check-updates')?.addEventListener('click', () => {
  ...
  window.oslo.checkForUpdates().then(info => {
    ...
    if (info.updateAvailable) {
      ...
      if (currentVersion) currentVersion.textContent = `v${info.currentVersion || '1.0.0-alpha.6'}`;
...
document.getElementById('btn-confirm-update')?.addEventListener('click', () => {
  ...
  const version = (document.getElementById('update-modal-version')?.textContent || '1.0.0-alpha.6').replace(/^v/, '');
